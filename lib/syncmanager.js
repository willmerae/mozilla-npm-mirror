...
SyncManager.prototype = {
      ...
      packages
        .map(function(pkg) {
          ...
          return versions.map(function(version) {
            return this.packageVersionSearch.bind(this, pkg, version);
          }.bind(this));
        }.bind(this))
  ...
  packageVersionSearch: function(pkg, version, callback) {
    ...
    if (!version || version === 'null' || UrlCheck.isWebUrl(version) ||
      ...
      delete this.packageToVersions[pkg][version];
      return callback();
    }
    ...
    async.waterfall([
      ...
      function(done) {
        var packageVersionUrl = Package.url(this.master, pkg, version);
  ...
  addDependencies: function(parent, depToVersions) {
    ...
    packages.forEach(function(pkg) {
      var currVersions = this.packageToVersions[pkg];
      var depVersions = depToVersions[pkg];
      newdeps[pkg] = {};

      // Check if we are watching this package.
      if (!currVersions) {
        // Add all of the versions.
        debug(parent + ' needs ' + pkg + '@' +
              Object.keys(depVersions).join(', '));
        this.packageToVersions[pkg] = depVersions;
        newdeps[pkg] = depVersions;
        return;
      }
      ...
      versions
        ...
        .forEach(function(version) {
          // Add this version.
          debug(parent + ' needs ' + pkg + '@' + version);
          this.packageToVersions[pkg][version] = true;
          newdeps[pkg][version] = true;
        }.bind(this));
    }.bind(this));
  ...
  makedirs: function(callback) {
    async.waterfall([
      ...
      function(done) {
        var dirs = [];
        for (var pkg in this.packageToVersions) {
          dirs.push(path.resolve(this.tempdir, pkg));
          var versions = this.packageToVersions[pkg];
          for (var version in versions) {
            dirs.push(path.resolve(this.tempdir, pkg, version));
          }
        }

        async.parallelLimit(dirs.map(function(dir) {
  ...
  downloadPackageRootObjects: function(callback) {
    ...
    async.parallelLimit(packages.map(function(pkg) {
      var packageRootUrl = url.resolve(this.master, pkg);
      return this.downloadPackageRootObject.bind(this, pkg, packageRootUrl);
    }.bind(this)),
    1,
    function(err) {
      return callback && callback(err);
  ...
  downloadPackageRootObject: function(pkg, packageRootUrl, callback) {
    ...
    async.waterfall([
      ...
      function(packageRootData, done) {
        ...
        var versions = Object.keys(this.packageToVersions[pkg]);
        versions.forEach(function(version) {
          ...
          if ('dist' in packageRootVersion) {
            copyVersion.dist = {
              ...
              tarball: Package.tarballUrl(this.hostname, pkg, version)
            };
          }
        ...
        if (latest && 'dist' in latest) {
          copy.dist = {
            ...
            tarball: Package.tarballUrl(this.hostname, pkg, latestVersion)
          };
        }

        var dest = path.resolve(this.tempdir, pkg, 'index.json');
    ...
    async.parallelLimit(
      packages
        .map(function(pkg) {
          var versions = Object.keys(packageToVersions[pkg]);
          return versions.map(function(version) {
            return this.downloadPackageVersion.bind(this, pkg, version);
          }.bind(this));
        }.bind(this))
        .reduce(function(prev, curr) {
          return prev.concat(curr);
  ...
  downloadPackageVersion: function(pkg, version, callback) {
    async.waterfall([
      // Check whether path exists.
      function(done) {
        var dest = path.resolve(this.tempdir, pkg, version, 'index.json');
    ...
    async.parallelLimit(
      packages
        .map(function(pkg) {
          var versions = Object.keys(packageToVersions[pkg]);
          return versions.map(function(version) {
            return this.verifyPackageVersion.bind(this, pkg, version);
          }.bind(this));
        }.bind(this))
        .reduce(function(prev, curr) {
          return prev.concat(curr);
  ...
  verifyPackageVersion: function(pkg, version, callback) {
    var manifest = path.resolve(this.tempdir, pkg, version, 'index.json');
    var packageVersion = require(manifest);
    if (!('dist' in packageVersion)) {
      return callback(new Error(pkg + '@' + version + ': no dist'));
    }
    if (!('shasum' in packageVersion.dist)) {
      return callback(new Error(pkg + '@' + version + ': no shasum'));
    }

    var expected = packageVersion.dist.shasum;
    var tarball = path.resolve(
      this.tempdir, pkg, version, pkg + '-' + version + '.tgz');
